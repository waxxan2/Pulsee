...
local function getEffectiveDurability(player)
    ...
    if ultimates then
        ...
        if infernal then
            multiplier = multiplier + infernal.Value; 
        end; 
    end;
...
local function blowsToKill(enemyHp, playerDmg)
    if not playerDmg or playerDmg <= 0 then
        return "N/A"; 
    end;
...
local function updateStats(player)
    ...
    statLabels["Strength"].Text = "Strength: " .. getStat(player, {
        "leaderstats",
        "Strength"
    });
    statLabels["Gems"].Text = "Gems: " .. getStat(player, {
        "Gems"
    });
    statLabels["Rebirth"].Text = "Rebirth: " .. getStat(player, {
        "leaderstats",
        "Rebirths"
    });
    statLabels["Agility"].Text = "Agility: " .. getStat(player, {
        "Agility"
    });
    statLabels["Durability"].Text = "Durability: " .. getStat(player, {
        "Durability"
    });
    statLabels["Kills"].Text = "Kills: " .. getStat(player, {
        "leaderstats",
        "Kills"
    });
    statLabels["Muscle King Time"].Text = "Muscle King Time: " .. getStat(player, {
        "muscleKingTime"
    });
    local cm = player:FindFirstChild("currentMap");
    statLabels["Current Map"].Text = "Current Map: " .. (cm and tostring(cm.Value) or "N/A");
    statLabels["Custom Size"].Text = "Custom Size: " .. getStat(player, {
        "customSize"
    });
    statLabels["Custom Speed"].Text = "Custom Speed: " .. getStat(player, {
        "customSpeed"
    });
    statLabels["Evil Karma"].Text = "Evil Karma: " .. getStat(player, {
        "evilKarma"
    });
    statLabels["Good Karma"].Text = "Good Karma: " .. getStat(player, {
        "goodKarma"
    });
